...
if __name__ == "__main__":
    ...
    print(X.shape)
    print(torch.bincount(torch.IntTensor(y).squeeze()))
    ...
    #     _X, y = RandomOverSampler().fit_resample(X.reshape(len(X), 3*32*32), y)
    #     X = _X.reshape(len(_X), 32, 32, 3)
    #     # indices, labels = RandomOverSampler().fit_resample(np.arange(len(X)).reshape(-1,1), y)
    #     # X, y = X[indices.flatten()], labels
    #     print(X.shape)
    # elif args.sampling_type == 'under':
    #     _X, y = RandomUnderSampler().fit_resample(X.reshape(len(X), 3*32*32), y)
    #     X = _X.reshape(len(_X), 32, 32, 3)
    #     print(X.shape)
    # elif args.sampling_type == 'smote':
    #     _X, y = SMOTE().fit_resample(X.reshape(len(X), 3*32*32), y)
    #     X = _X.reshape(len(_X), 32, 32, 3)
    #     print(X.shape)
    ...
    # evens = list(range(0, len(full_dataset), 2))
    # odds = list(range(1, len(full_dataset), 2))
    # train_set = torch.utils.data.Subset(full_dataset, evens)
    # test_set = torch.utils.data.Subset(full_dataset, odds)
    ...
    print(torch.bincount(torch.IntTensor(y_train).squeeze()))
    print(torch.bincount(torch.IntTensor(y_test).squeeze()))
    print(y_train)
    print(y_test)
    print(X_train.shape)
    print(X_test.shape)
    ...
    print('epsilon, acc, train acc, mia, min mia, maj mia, report, train report')
    results = [epsilon, acc, train_acc, mlp_acc_bb, total_min_acc, total_maj_acc, class_report, train_class_report]
